	export class Bullet {
	    constructor(scene, position, velocity) {
	        ...
	        try {
	            ...
	            // Try to load the texture, but don't crash if it fails
	            try {
	                this.particleSystem.particleTexture = new BABYLON.Texture("textures/flare.png", scene);
	            } catch (e) {
	                console.warn("Particle texture not found, using default texture");
	    ...
	    createTrail() {
	        ...
	        for (let i = 0; i < this.maxTrailLength; i++) {
	            try {
	                ...
	                console.error("Failed to create trail segment:", e);
	                this.particles.push(null);
	            }
	        }
	    }
	    update() {
	        // Check if bullet has been disposed
	        ...
	        try {
	            ...
	            this.particles.forEach((segment, i) => {
	                if (segment && i < this.trailPositions.length - 1) {
